...
public interface UserVelogUserRepository extends JpaRepository<UserVelogUser, Long> {
    // Todo: 리턴타입을 Optional로 수정했으므로, 관련 로직들 추후 수정하기
    Optional<UserVelogUser> findByUser(User user);
    Optional<UserVelogUser> getByUserAndVelogUser(User user, VelogUser velogUser);
}